...
const projectDir = path.resolve(__dirname, '../../web-dev/src/public/utils/wechat-emoji-parser')
...
function modifyTsDefinePath() {
  ...
  if (projectDir) {
    const jsFile = path.resolve(targetDir, 'index.js')
    const targetJsFile = path.resolve(projectDir, 'index.js')
    let jsContent = fs.readFileSync(jsFile).toString()
    fs.writeFileSync(targetJsFile, jsContent)
  }
}
